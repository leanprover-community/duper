...
partial def toLeanExpr (t : Parser.Term) : MetaM Expr := do
  ...
  | ⟨.ident "$tType", []⟩ => return mkSort Level.one
  | ⟨.ident "$o", []⟩ => return mkSort Level.zero
  ...
  | ⟨.op "!=", []⟩  => pure $ mkConst `Ne
  | ⟨.op "=", []⟩   => pure $ mkConst `Eq
  | ⟨.op "~|", []⟩  => pure $ mkLambda `x .default (mkSort Level.zero) $
  ...
  | ⟨.ident n, as⟩ => do
    ...
    then
      let ty ← as.foldlM
        ...
        (if topLevel then mkSort Level.zero else mkConst `Iota)
