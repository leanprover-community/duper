...
--example (x y z : Nat) (f : Nat → Nat) (H1 : ∀ x, f x = f y) (H2 : f y = f z)
--  (H3 : f x ≠ f z) : False := by duper
--
...
--
--set_option trace.Rule.boolHoist true in
--example (f : Prop → Nat) :
...
set_option trace.Meta.debug true in
example : (∀ (A : Type) (x1 : A), x1 = x1) = True :=
  have : Inhabited Type := ⟨Nat⟩
  by duper
