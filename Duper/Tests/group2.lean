...
by duper [Group.mul_one] {portfolioInstance := 1}

theorem exists_right_inv (x : G) : inv x ⬝ x = one :=
by duper [Group.mul_assoc, Group.mul_one, Group.mul_inv] {portfolioInstance := 1}
...
theorem sq_mul_sq_eq_e (h_comm : ∀ (a b : G), a ⬝ b = b ⬝ a) (h : x ⬝ y = one) :
  sq x ⬝ sq y = one :=
by duper [h, h_comm, sq, Group.mul_assoc, Group.mul_one, Group.mul_inv] {portfolioInstance := 1}
