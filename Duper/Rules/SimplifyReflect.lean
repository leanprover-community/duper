        ...
        let pr : Expr ← Meta.withLocalDeclD `h lit.toExpr fun h => do
          if(i == mainPremisePos.lit) then
            ...
            if mainPremisePos.side == LitSide.lhs then -- the lhs of the side clause can be matched onto mainPremisePos.pos of lit.lhs
              ...
              Meta.mkLambdaFVars #[h] $ mkApp2 (mkConst ``False.elim [Level.zero]) body $ ← Meta.mkAppM' hAfterRw #[← Meta.mkAppM ``Eq.refl #[lit.rhs]]
              ...
              let hAfterRw ← Meta.mkAppOptM ``Eq.ndrec #[none, none, motive, h, none, appliedSidePremise]
              Meta.mkLambdaFVars #[h] $ mkApp2 (mkConst ``False.elim [Level.zero]) body $ ← Meta.mkAppM' hAfterRw #[← Meta.mkAppM ``Eq.refl #[lit.lhs]]
        ...
        let pr : Expr ← Meta.withLocalDeclD `h lit.toExpr fun h => do
          if(i == mainPremiseLitIdx) then
            if mainPremiseLhs == LitSide.lhs then
              Meta.mkLambdaFVars #[h] $ mkApp2 (mkConst ``False.elim [Level.zero]) body $ mkApp appliedSidePremise h
            else
              Meta.mkLambdaFVars #[h] $ mkApp2 (mkConst ``False.elim [Level.zero]) body $ mkApp (← Meta.mkAppM ``Ne.symm #[appliedSidePremise]) h
