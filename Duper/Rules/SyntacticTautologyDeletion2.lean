...
def syntacticTautologyDeletion2 : MSimpRule := fun c => do
  let mut eq_pairs : Std.HashSet (Lean.Expr × Lean.Expr) := Std.HashSet.empty; -- A HashSet that stores pairs of Lean expressions that are equated in the clause
  let mut ne_pairs : Std.HashSet (Lean.Expr × Lean.Expr) := Std.HashSet.empty; -- A HashSet that stores pairs of Lean expressions that are stated to not be equal in the clause
