...
(goalDistance : Nat) -- min (maxGoalDistance, the minimum number of inference (not simplification) rules between the clause and the goal)
(proof : Proof)
...
def addNewToPassive (c : Clause) (proof : Proof) (goalDistance : Nat) (generationNumber : Nat) (generatingAncestors : Array Clause) : ProverM Unit := do
  ...
  | some ci =>
    if (ci.wasSimplified) then pure () -- No need to add c to the passive set because it would just be simplified away later
      ...
      setPassiveSet $ (← getPassiveSet).insert c (c.selectionPrecedence goalDistance) ci.generationNumber ci.number
